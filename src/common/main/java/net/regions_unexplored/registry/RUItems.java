...
import static net.regions_unexplored.item.RUItemUtils.consumable;
import static net.regions_unexplored.item.RUItemUtils.food;

public interface RUItems {
    Supplier<Item> SALMONBERRY = RUItemUtils.register("salmonberry", p -> new BlockItem(RUBlocks.SALMONBERRY_BUSH.get(),
        p.food(food(3, 0.3f), consumable(t -> t))
    ));
    Supplier<Item> DUSKMELON_SLICE = RUItemUtils.register("duskmelon_slice", p -> new BlockItem(RUBlocks.DUSKMELON.get(),
        p.food(food(5, 1.1f), consumable(t -> t.onConsume(new ApplyStatusEffectsConsumeEffect(new MobEffectInstance(MobEffects.BLINDNESS, 240), 1))))
    ));
    Supplier<Item> HANGING_EARLIGHT_FRUIT = RUItemUtils.register("hanging_earlight_fruit", p -> new BlockItem(RUBlocks.HANGING_EARLIGHT.get(),
